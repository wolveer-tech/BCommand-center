...
Version **1.7.0 (build 13)** changes Privacy Lock from a scene-based relock
timer to a cold-launch lock. Control Centre, Notification Centre and the app
switcher still receive a privacy shield, but returning to an already unlocked
app does not request Face ID again. **Lock now** remains an explicit relock.

The native Mirror bridge also accepts a picker retry action. After a room is
created, the web interface can reopen Apple's ReplayKit broadcast picker if it
was dismissed or did not appear. Native signalling retries a colliding room
code rather than silently replacing an active room.

Build and install this IPA over the existing app. Do not delete the old app
first: an in-place install is important for retaining the Keychain-backed
Messages and Transfers identity. No entitlement, App Group, database migration
or Cloudflare secret is added.
